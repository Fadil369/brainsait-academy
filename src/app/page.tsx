          ...
          <div className="space-y-6">
                ...
                <AlertDialogContent className="rounded-[1.4rem] border-border/70 bg-popover p-0 text-popover-foreground sm:max-w-xl">
                  <div className="p-6">
                    <AlertDialogHeader className="items-start text-left">
                      ...
                      <AlertDialogTitle className="font-headline text-2xl font-semibold">A more premium academy shell</AlertDialogTitle>
                      <AlertDialogDescription>
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-3">
                      {[
                        { label: "Course library", value: `${coursesData.length}` },
                        { label: "Active tracks", value: `${activeTopics.length}` },
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2 text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                <span>Trusted by quality, safety, and transformation teams</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 font-headline text-3xl font-semibold text-foreground">Sharper theme, stronger trust, cleaner discovery.</div>
                </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  { icon: ShieldCheck, value: `${activeTopics.find((topic) => topic.slug === "patient-safety")?.count || 0}`, label: "Patient safety courses" },
                  { icon: BookOpenCheck, value: `${enrolledCount}`, label: "Saved learning journeys" },
                  { icon: Languages, value: "AR / EN", label: "Bilingual delivery" },
                  { icon: Bot, value: "AI-ready", label: "Modern academy stack" },
                ].map((item) => (
              ...
              <div className="rounded-[1.25rem] border border-border/70 bg-background/70 p-4">
                <div className="mb-3 text-xs uppercase tracking-[0.18em] text-muted-foreground">Featured premium tracks</div>
                    ...
                    return (
                      <Link key={topic.slug} href={`/topics/${topic.slug}`} className="flex items-center justify-between gap-3 rounded-2xl border border-border/60 bg-surface-container-low px-4 py-3 transition-colors hover:border-secondary/30 hover:bg-surface-container">
                        <div className="flex items-center gap-3">
                          <div className={cn("flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br font-headline text-sm font-semibold text-white", branding.gradient)}>
                            {branding.mark}
                          </div>
                          <div>
                            <div className="font-medium text-foreground">{topic.name}</div>
                            <div className="text-xs text-muted-foreground">{topic.count} curated courses</div>
                          </div>
                        </div>
        ...
        <div className="mesh-panel -mt-4 p-4 lg:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-2">
              ...
              <div className="font-headline text-2xl font-semibold text-foreground">Find the right course, fast.</div>
            </div>
            ...
            <div className="flex w-full max-w-3xl items-center gap-3 rounded-full border border-border/70 bg-background/80 px-3 py-2 backdrop-blur-sm">
              ...
              <Input
                ...
                placeholder="Search courses, topics, clinical themes, or Arabic titles"
      ...
      <main className="max-w-7xl mx-auto space-y-14 px-container-padding py-12 pb-28">
        <section className="space-y-5">
          <div className="flex items-end justify-between gap-4">
            <div>
              <div className="section-kicker">Signature learning tracks</div>
              <h2 className="mt-3 font-headline text-3xl font-semibold text-foreground lg:text-4xl">A more premium visual identity for every specialty lane.</h2>
            </div>
            <Link href="/topics" className="hidden md:inline-flex">
              <Button variant="ghost" className="rounded-full text-foreground">View all tracks</Button>
            </Link>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {featuredTopics.map((topic) => {
              const branding = getTopicBranding(topic.name);
              return (
                <Link key={topic.slug} href={`/topics/${topic.slug}`} className="premium-card card-shine rounded-[1.5rem] p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-3">
                      <div className={cn("inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br font-headline text-sm font-semibold tracking-[0.16em] text-white", branding.gradient)}>
                        {branding.mark}
                      </div>
                      <div>
                        <h3 className="font-headline text-2xl font-semibold text-foreground">{topic.name}</h3>
                        <p className="mt-2 max-w-sm text-sm leading-7 text-muted-foreground">{branding.description}</p>
                      </div>
                    </div>
                    <Badge className={cn("rounded-full border-0 px-3 py-1", branding.badgeClass)}>{topic.count} courses</Badge>
                  </div>
                </Link>
              );
            })}
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          {[
            ...
            <Card key={item.title} className="premium-card rounded-[1.5rem] border-border/70 bg-card shadow-none">
        ...
        <section id="catalog" className="space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="section-kicker">Course collection</div>
              <h2 className="mt-3 font-headline text-3xl font-semibold text-foreground lg:text-4xl">
                Curated catalog with richer cards, clearer marks, and stronger discovery.
              </h2>
            </div>
          ...
          {filteredCourses.length > visibleCourses.length && (
            <div className="flex justify-center">
              <Link href="/topics">
                <Button variant="outline" className="rounded-full px-6">See the full academy</Button>
              </Link>
            </div>
          )}

          {filteredCourses.length === 0 && (
            <div className="premium-card rounded-[1.5rem] p-10 text-center">
              <div className="font-headline text-3xl font-semibold text-foreground">Nothing matched that search.</div>
