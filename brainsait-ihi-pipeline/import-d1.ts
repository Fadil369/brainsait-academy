...
function serializeCourse(course: CourseRecord): string {
  return JSON.stringify(course.sections);
}

function buildSql(courses: CourseRecord[]): string {
  ...
  const insertStatements = courses.map(
    (course) => `INSERT OR REPLACE INTO courses (
  slug,
  title,
  title_arabic,
  source_url,
