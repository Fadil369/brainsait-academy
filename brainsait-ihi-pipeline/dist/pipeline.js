...
function parseBilingualSections(body) {
    const lines = body.split('\n');
    const sections = [];
    let currentHeading = 'Overview';
    let currentHeadingArabic = '';
    let currentContent = [];
    let currentContentArabic = [];
...
function extractEnrichmentMetadata(body, keywords) {
    const saudiTerms = ['saudi', 'moh', 'ministry of health', 'cbahi', 'vision 2030', 'riyadh', 'jeddah', 'ال سعود', 'وزارة الصحة', 'هيئة'];
    const hasSaudiContext = saudiTerms.some(term => body.toLowerCase().includes(term.toLowerCase()));
    const roleTerms = ['physician', 'nurse', 'pharmacist', 'quality', 'الأطباء', 'التمريض', 'الصيدلة'];
    const roleBasedContent = roleTerms.filter(term => body.toLowerCase().includes(term.toLowerCase()));
