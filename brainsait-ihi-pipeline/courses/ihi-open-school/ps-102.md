---
title: "From Error to Harm"
titleArabic: "PS 102: من الخطأ إلى الأذى"
slug: "ps-102"
sourceUrl: "https://www.ihi.org/learn/courses/open-school/catalog/ps-102"
status: Published
duration: 60
keywords: Patient Safety, IHI, Open School, Health Care, PS
---

# From Error to Harm

**PS 102: من الخطأ إلى الأذى**

## Overview

Learn about the relationship between error and harm using the Swiss cheese model of error.

## نظرة عامة

learn حول العلاقة بين الخطأ والأذى باستخدام نموذج الجبن السويسري للخطأ.

## Course Details

- **Code:** PS 102
- **Topic:** Patient Safety
- **Format:** Online, On-Demand
- **Provider:** IHI Open School
- **Source:** [IHI Education Platform](https://www.ihi.org/learn/courses/open-school/catalog/ps-102)

## تفاصيل الدورة

- **الرمز:** PS 102
- **الموضوع:** Patient Safety
- **الشكل:** عبر الإنترنت، حسب الطلب
- **المزود:** IHI Open School
- **المصدر:** [منصة IHI التعليمية](https://www.ihi.org/learn/courses/open-school/catalog/ps-102)

## Learning Objectives

Upon completion of this course, learners will be able to:

1. Understand the core concepts related to patient safety
2. Apply improvement methodologies in health care settings
3. Demonstrate competency in the course subject matter
4. Earn Continuing Education (CE) credits

## أهداف التعلم

بعد إتمام هذه الدورة، سيتمكن المتعلمون من:

1. فهم المفاهيم الأساسية المتعلقة بـ patient safety
2. تطبيق منهجيات التحسين في بيئات الرعاية الصحية
3. إظهار الكفاءة في موضوع الدورة
4. كسب ساعات التعليم المستمر (CE)

## Key Takeaways

- Practical skills for health care improvement
- Evidence-based approaches to quality and safety
- Frameworks for implementing change in health care organizations

## النقاط الرئيسية

- مهارات عملية لتحسين الرعاية الصحية
- نهج قائمة على الأدلة للجودة والسلامة
- أطر عمل لتنفيذ التغيير في منظمات الرعاية الصحية
